...
ConjunctiveNormalForm Gate::characteristicFunction()
{
    ...
    {
    case Type::AND:
        literals.push_back({m_nets.at(0)->number(), true, INITIAL_VALUE});
        literals.push_back({m_nets.at(1)->number(), true, INITIAL_VALUE}); 
        literals.push_back({m_nets.at(2)->number(), false, INITIAL_VALUE});

        cnf.addClausesFromLiterals(literals);
        literals.clear();
         
        literals.push_back({m_nets.at(0)->number(), false, INITIAL_VALUE}); 
        literals.push_back({m_nets.at(2)->number(), true, INITIAL_VALUE});
        
        cnf.addClausesFromLiterals(literals);
        literals.clear();

        literals.push_back({m_nets.at(1)->number(), false, INITIAL_VALUE}); 
        literals.push_back({m_nets.at(2)->number(), true, INITIAL_VALUE});
